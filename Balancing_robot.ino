...
int acc_calibration_value = -7808;                            //Enter the accelerometer calibration value

//Various settings
float pid_p_gain = 35;                                        //Gain setting for the P-controller (30) - 35
float pid_i_gain = 0.68;                                      //Gain setting for the I-controller (1.5) - 0.5
float pid_d_gain = 3;                                       //Gain setting for the D-controller (15) - 3
float turning_speed = 150;                                    //Turning speed (20)
...
//  pinMode(13, OUTPUT);                                                      //Configure digital poort 6 as output
  ...
  for(receive_counter = 0; receive_counter < 520; receive_counter++){       //Create 500 loops
//    if(receive_counter % 15 == 0)digitalWrite(13, !digitalRead(13));        //Change the state of the LED every 15 loops to make the LED blink fast
    ...
    delayMicroseconds(3500);                                                //Wait for 3700 microseconds to simulate the main program loop time
  }
  gyro_pitch_calibration_value /= 520;                                      //Divide the total value by 500 to get the avarage gyro offset
  gyro_yaw_calibration_value /= 520;                                        //Divide the total value by 500 to get the avarage gyro offset
...
//  }
  ...
  if(received_byte != 0) {
    if(received_byte == 49){                                            //If the first bit of the receive byte is set change the left and right variable to turn the robot to the left
//      Serial.println("esquerda");
      pid_output_left += turning_speed;                                       //Increase the left motor speed
      pid_output_right -= turning_speed;                                      //Decrease the right motor speed
    }
    if(received_byte == 51){                                            //If the second bit of the receive byte is set change the left and right variable to turn the robot to the right
//      Serial.println("direita");
      pid_output_left -= turning_speed;                                       //Decrease the left motor speed
      pid_output_right += turning_speed;                                      //Increase the right motor speed
    }
  
    if(received_byte == 52){                                            //If the third bit of the receive byte is set change the left and right variable to turn the robot to the right
//      Serial.println("frente");
      if(pid_setpoint > -2.5)pid_setpoint -= 0.15;                            //Slowly change the setpoint angle so the robot starts leaning forewards
      if(pid_output > max_target_speed * -1)pid_setpoint -= 0.015;            //Slowly change the setpoint angle so the robot starts leaning forewards
    }
    if(received_byte == 50){                                            //If the forth bit of the receive byte is set change the left and right variable to turn the robot to the right
//      Serial.println("tras");
      if(pid_setpoint < 2.5)pid_setpoint += 0.15;                             //Slowly change the setpoint angle so the robot starts leaning backwards
      if(pid_output < max_target_speed)pid_setpoint += 0.015;                 //Slowly change the setpoint angle so the robot starts leaning backwards
    }  
...
//    }
  ...
  if(pid_setpoint == 0){                                                    //If the setpoint is zero degrees
    if(pid_output < 0)self_balance_pid_setpoint += 0.001;                  //Increase the self_balance_pid_setpoint if the robot is still moving forewards
    if(pid_output > 0)self_balance_pid_setpoint -= 0.001;                  //Decrease the self_balance_pid_setpoint if the robot is still moving backwards
  }
